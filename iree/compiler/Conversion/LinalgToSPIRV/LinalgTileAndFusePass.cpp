...
static Optional<linalg::TiledAndFusedLinalgOps> tileAndFuseLinalgOps(
  ...
  if (fusableOps.size() == 1) {
    ...
    for (auto size : enumerate(ArrayRef<int64_t>(tileSizes).take_front(
             getNumOuterParallelLoops(fusableOps.front())))) {
      if (size.value() != 0)
        tiledAndFusedOps->fusedLoopDims.insert(size.index());
    }
